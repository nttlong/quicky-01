            ...
            for data_item in data:
                try:
                    ...
                    if ex.parent_caption != None:
                        ...
                        raise exceptions.MissingFields(ex.fields, [index,ex.index], data_item, parent_name+"["+index.__str__()+"]." +endfix)
                    else:
        ...
        for item in list(check_fields):
            if data[item]!= None:
                if self.detail[item].data_type != type(data[item]):
                    if parent_name!=None:
                        raise exceptions.InvalidDataFields(parent_name+"." +item,type(data[item]),self.detail[item].data_type)
                if isinstance(data[item],dict):
                    ...
                    for data_item in data[item]:
                        try:
                            ...
                            if ex.parent_caption == None:
                                raise exceptions.InvalidDataFields(ex.fields,ex.receive_data_type,ex.expected_data_type,index,data_item,parent_name+"."+item+"["+index.__str__()+"]")
                            else:
    ...
    def execute(self,*args,**kwargs):
        if self.param_by_index:
            ...
            for item in args:
                ...
                index += 1
            if self.validate(data_params):
                self.__caller__(*args, **kwargs)
        else:
