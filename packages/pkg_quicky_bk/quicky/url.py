...
def build_urls(module_name, *args, **kwargs):
    ...
    if _apps_ == None:
        ...
        if not is_multi_tenancy:
            for app in args[0]:
                try:
                    if host_dir == None:
                        ...
                        if ret.host_dir == "":
                            _apps_.urlpatterns.append(
                                url(r"^(?i)" + host_dir + "/", include(ret.mdl.__name__ + ".urls")))
                        else:
                            _apps_.urlpatterns.append(
                                url(r"^(?i)" + host_dir + "/" + ret.host_dir + "/",
                                    include(ret.mdl.__name__ + ".urls")))
                except Exception as ex:
            ...
            list_of_key_remain_apps = [x.get("host") for x in args[0] if
                                       list_of_key_of_apps_with_persistent_schema.count(
                                           x.get("host")) == 0 and list_of_key_of_apps_with_public.count(
                                           x.get("host")) == 0]
                ...
                if dict_utils.has_key(app, "authenticate"):
                    try:
                        ...
                        if not callable(_authenticate):
                            raise (Exception(
                                "'{0}' is callable. Please, check application with name '{1}' in settings.py".format(
                                    app.get("authenticate"), app.get("name"))))
                        setattr(ret.settings, "authenticate", app.get("login"))
                    except Exception as ex:
                        msg = "Can not import '{0}'. Please, check application with name '{1}' in settings.py ".format(
                            app.get("authenticate"), app.get("name"))
                        raise (Exception(msg))
                            ...
                            if ret.host_dir == "" and not ret.is_persistent_schema():
                                ...
                                if host_dir == None:
                                    ...
                                    url_regex = url_regex.replace("^",
                                                                  "^(?i)(?P<tenancy_code>" + get_tenancy_code_regex() + ")/")
                                else:
                                    url_regex = url_regex.replace("^",
                                                                  "^(?i)" + host_dir + "/(?P<tenancy_code>" + get_tenancy_code_regex() + ")/")
