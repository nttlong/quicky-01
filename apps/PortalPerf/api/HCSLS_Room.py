...
def get_list_with_searchtext(args):
    ...
    items = models.HCSLS_Room().aggregate()
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_Room().insert(data)
            lock.release()
            return ret
...
def set_dict_insert_data(args):
    ...
    ret_dict.update(
        ...
        room_name=(lambda x: x['room_name'] if x.has_key('room_name') else None)(args['data']),
        room_name2=(lambda x: x['room_name2'] if x.has_key('room_name2') else None)(args['data']),
        supplier_code=(lambda x: x['supplier_code'] if x.has_key('supplier_code') else None)(args['data']),
        room_map=(lambda x: x['room_map'] if x.has_key('room_map') else None)(args['data']),
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_Room().update(
                data, 
                "_id == {0}", 
                ObjectId(args['data']['_id']))
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = models.HCSLS_Room().aggregate().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret = models.HCSLS_Room().delete("room_code in {0}", [x["room_code"] for x in args['data']])
            lock.release()
            return ret
