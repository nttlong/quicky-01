(function(scope){
    ...
    scope.$$table1={
        tableFields: [
            { "data": "target_name", "title": "${get_res('approver','Người duyệt')}", "className": "text-left" },
            { "data": "unit_name", "title": "${get_res('approve_date','Ngày duyệt')}", "className": "text-left" },
            { "data": "weight", "title": "${get_res('content','Nội dung')}", "className": "text-left" },
            { "data": "target", "title": "${get_res('status_approve','Trạng thái duyệt')}", "className": "text-left" },
            { "data": "min_value", "title": "${get_res('status_ticket','Trạng thái phiếu')}", "className": "text-center" }
        ]
    }
    ...
    scope.$$table = {
        tableFields: [
            ...
            { "data": "status", "title": "${get_res('status','Trạng thái')}", "className": "text-center"},
            { "data": "approve", "title": "${get_res('approve','Xét duyệt')}", "className": "text-center"}
        ],
        $$tableConfig: {},
