﻿(function (scope) {
    ...
    scope.$root.approveTopic = function () {
         if (scope.currentItem) {
            scope.mode = 2; // set mode chỉnh sửa
            scope.topic_approved = true
            openDialog("${get_res('Approve_Topic','Approve Topic')}", 'form/appTopic', function () {
                setTimeout(function () {
    ...
    scope.$root.delete = function () {
        ...
        if (scope.selectedItems.length > 0) {
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.LMSLS_Topic/delete_to_trash')}")
                    .data(arrayId)
                    .done()
                    .then(function (res) {
    ...
    scope.tableFields = [
        ...
        { "data": "created_on", "title": "${get_res('created_by','Created by')}","expr":function(row, data, func){
            func(function(){
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback, objSearchAdvance) {
        //if (!scope.treeCurrentNode.forum_id) return;
        var sort = {};
        $.each(orderBy, function (i, v) {
            sort[v.columns] = (v.type === "asc") ? 1 : -1;
        });
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.LMSLS_Topic/get_list_with_searchtext_pending')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
