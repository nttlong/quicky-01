...
from services import LMS_SurQuestionBankService as service
...
def insert(args):
    return service.insert_sur_question_bank(args['data'])

@authorization.authorise(action = action_type.Action.WRITE)
def update(args):
    return service.update_sur_question_bank(args['data'])

@authorization.authorise(action = action_type.Action.DELETE)
def delete(args):
    return service.delete_sur_question_bank(args['data'])
