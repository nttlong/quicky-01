...
//            data_year: data_data
    ...
    scope.SearchText = '';
//    scope.$parent.$parent.advancedSearch.data_year ='';
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_TrainRequest/get_list_with_searchtext')}")
                .data({
                    ...
                    "search": searchText,
//                    "where": {
////                            'year': scope.advancedSearch.data_lock,
//                              'year': scope.advancedSearch.$$$filter_active
//                        },
                    "sort": sort,
                    "filler_year": scope.$parent.$parent.advancedSearch.data_year,
    ...
    debugger
        ...
        openDialog("${get_res('Create_request','Chi tiết yêu cầu đào tạo')}", 'form/addTrainRequest', function () {
            setTimeout(function () {
    ...
    */
    scope.editRoom = scope.$parent.$parent.$parent.editRoom = function () {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Edit_request','Chi tiết yêu cầu đào tạo')}", 'form/addTrainRequest', function () { });
        } else {
    ...
    */
    ...
    scope.$parent.$watch("advancedSearch.data_year", function (val) {
    debugger
        var config = scope.$$tableConfig;
        _tableData(config.iPage, config.iPageLength, config.orderBy, config.searchText, config.fnReloadData);
    });
