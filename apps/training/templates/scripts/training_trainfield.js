(function (scope) {
    ...
    scope.tableFields = [
        { "data": "field_code", "title": "${get_res('field_code','Mã Lĩnh Vực')}" },
        { "data": "field_name", "title": "${get_res('field_name','Tên Lĩnh Vực')}" },
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_TrainField/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
    ...
    scope.$parent.$parent.$parent.addRoom = function () {
        ...
        openDialog("${get_res('Create_Filed','Add Train Field')}", 'form/addTrainField', function () {
            setTimeout(function () {
    ...
    */
    scope.editRoom = scope.$parent.$parent.$parent.editRoom = function () {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Edit_Field','Edit Train Field')}", 'form/addTrainField', function () { });
        } else {
    ...
    scope.$parent.$parent.$parent.delRoom = function () {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_TrainField/delete')}")
