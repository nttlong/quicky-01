...
def get_list_with_searchtext(args):
    ...
    items = models.HCSLS_Equipment().aggregate()
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_Equipment().insert(data)
            lock.release()
            return ret
...
def set_dict_insert_data(args):
    ...
    ret_dict.update(
        equipment_code          = (lambda x: x['equipment_code']        if x.has_key('equipment_code')          else None)(args['data']),
        equipment_name          = (lambda x: x['equipment_name']        if x.has_key('equipment_name')          else None)(args['data']),
        equipment_name2          = (lambda x: x['equipment_name2']        if x.has_key('equipment_name2')          else None)(args['data']),
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_Equipment().update(
                data, 
                "_id == {0}", 
                ObjectId(args['data']['_id']))
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = models.HCSLS_Equipment().aggregate().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  models.HCSLS_Equipment().delete("equipment_code in {0}",[x["equipment_code"]for x in args['data']])
            lock.release()
            return ret
