...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_TrainField().insert(data)
            lock.release()
            return ret
...
def set_dict_insert_data(args):
    ...
    ret_dict.update(
        ...
        field_name2=(lambda x: x['field_name2'] if x.has_key('field_name2') else None)(args['data']),
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_TrainField().update(
                data, 
                "_id == {0}", 
                ObjectId(args['data']['_id']))
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = models.HCSLS_TrainField().aggregate().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret = models.HCSLS_TrainField().delete("field_code in {0}", [x["field_code"] for x in args['data']])
            lock.release()
            return ret
