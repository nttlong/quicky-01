...
def get_list_with_searchtext(args):
    ...
    data_year = args['data'].get('filler_year', '')
    ...
    items = models.HCSLS_TrainRequest().aggregate()
    ...
    items = common.filter_lock(items, args);
    if(data_year != ''):
        items.match("contains(year, @name)", name=searchText)
        # items.match("contains(request_code, @name) or contains(course_name, @name)" + \
        #             " or contains(reason_train, @name) or contains(year, @name)", name=data_year)
        # items.match("(year ={0})",data_year)
        # items.match("(end_date>={0})", dt_current)
    # items = common.filter_year(items, args);
    # if (data_year != None):
    #     items.match("contains(year, @name) or " + \
    #               "contains(ordinal, @name)", name=data_year.strip())
    if (searchText != None):
        items.match("contains(request_code, @name) or contains(course_name, @name)" + \
                  " or contains(reason_train, @name) or contains(year, @name)", name=searchText.strip())

    if(sort != None):
        items.sort(sort)

    # if args['data'].has_key('where'):
    #     return items.match(args['data']['where']['year'], pageSize, pageIndex, sort,(lambda x: x.strip() if x != None else "")(searchText))
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_TrainRequest().insert(data)
            lock.release()
            return ret
...
def set_dict_insert_data(args):
    ...
    ret_dict.update(
        request_code=(lambda x: x['request_code'] if x.has_key('request_code') else None)(args['data']),
        course_name=(lambda x: x['course_name'] if x.has_key('course_name') else None)(args['data']),
        reason_train=(lambda x: x['reason_train'] if x.has_key('reason_train') else None)(args['data']),
        ...
        month=(lambda x: x['month'] if x.has_key('month') else None)(args['data']),
        cost_estimate=(lambda x: x['cost_estimate'] if x.has_key('cost_estimate') else None)(args['data']),
        address=(lambda x: x['address'] if x.has_key('address') else None)(args['data']),
        department_request=(lambda x: x['department_request'] if x.has_key('department_request') else None)(args['data']),
        proponent=(lambda x: x['proponent'] if x.has_key('proponent') else None)(args['data']),
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  models.HCSLS_TrainRequest().update(
                data, 
                "_id == {0}", 
                ObjectId(args['data']['_id']))
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = models.HCSLS_TrainRequest().aggregate().match("_id == {0}", ObjectId(args['data']['_id'])).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret = models.HCSLS_TrainRequest().delete("request_code in {0}", [x["request_code"] for x in args['data']])
            lock.release()
            return ret
