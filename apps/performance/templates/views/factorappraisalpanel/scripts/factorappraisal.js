(function (scope) {
    ...
    function onAdd() {
        ...
        openDialog("${get_res('Factor_Appraisal_Detail','Chi tiết yếu tố đánh giá')}", 'factorappraisalpanel/form/addFactorAppraisal', function () { });
    };
    function onEdit() {
        if (scope.$$table.currentItem && Object.keys(scope.$$table.currentItem).length > 0) {
            scope.mode = 2; // set mode chỉnh sửa
            openDialog("${get_res('Edit_Domain','Chi tiết yếu tố đánh giá')}", 'factorappraisalpanel/form/addFactorAppraisal', function () { });
        } else {
