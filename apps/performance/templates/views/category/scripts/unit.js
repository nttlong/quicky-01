(function (scope) {
    ...
    function onDelete() {
        if (!scope.selectedItems || scope.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_Unit/delete')}")
    ...
    function onCopy() {
        if (scope.currentItem) {
            ...
            openDialog("${get_res('Detail_Unit','Chi tiết đơn vị tính')}", 'category/form/addEmployeeType', function () { });
        } else {
            $msg.message("${get_global_res('Notification','Thông báo')}", "${get_global_res('No_Row_Selected','Không có dòng được chọn')}", function () { });
        }
    }
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_Unit/get_list_with_searchtext')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
