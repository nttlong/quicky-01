(function (scope) {
    ...
    function onAdd() {
        ...
        openDialog("${get_res('Detail_Evaluation_Factor_By_Job_Working','Chi tiết yếu tố đánh giá theo chức danh')}", 'jobworkingpanel/jobworkingdetail/form/addEvaluationFactor', function () { });
    };
    function onEdit() {
    ...
    function onDelete() {
        if (!scope.$$table.selectedItems || scope.$$table.selectedItems.length === 0) {
            ...
            $msg.confirm("${get_global_res('Notification','Thông báo')}", "${get_global_res('Do_You_Want_Delete','Bạn có muốn xóa không?')}", function () {
                services.api("${get_api_key('app_main.api.HCSLS_JobWorking/delete_factor_appraisal')}")
                    .data({
    ...
    function _tableData(iPage, iPageLength, orderBy, searchText, callback) {
        ...
        sort[orderBy[0].columns] =
            services.api("${get_api_key('app_main.api.HCSLS_JobWorking/get_list_evaluation_factor')}")
                .data({
                    //parameter at here
                    "pageIndex": iPage - 1,
