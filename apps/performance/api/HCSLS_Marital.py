...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(marital_name, @name) or " + \
            "contains(marital_code, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Marital.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Marital.update(
                data, 
                "marital_code == {0}", 
                args['data']['marital_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = Marital.display_list_marital().match("marital_code == {0}", args['data']['marital_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Marital.delete("marital_code in {0}",[x["marital_code"]for x in args['data']])
            lock.release()
            return ret
