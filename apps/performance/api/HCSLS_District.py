...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(district_name, @name) or " + \
            "contains(district_code, @name) or " + \
            "contains(org_district_code, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_District.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_District.update(
                data, 
                "district_code == {0}", 
                args['data']['district_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = AdministrativeSubdivisions.get_district().match("district_code == {0}", args['data']['district_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_District.delete("district_code in {0}",[x["district_code"]for x in args['data']])
            lock.release()
            return ret
