...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(religion_name, @name) or " + \
            "contains(religion_code, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_Religion.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_Religion.update(
                data, 
                "religion_code == {0}", 
                args['data']['religion_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = Religion.display_list_religion().match("religion_code == {0}", args['data']['religion_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Religion.delete("religion_code in {0}",[x["religion_code"]for x in args['data']])
            lock.release()
            return ret
