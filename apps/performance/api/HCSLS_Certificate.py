...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Certificate.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Certificate.update(
                data, 
                "cer_code == {0}", 
                args['data']['cer_code'])
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Certificate.delete("cer_code in {0}",[x["cer_code"]for x in args['data']])
            lock.release()
            return ret
