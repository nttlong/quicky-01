...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Ward.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Ward.update(
                data, 
                "ward_code == {0}", 
                args['data']['ward_code'])
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Ward.delete("ward_code in {0}",[x["ward_code"]for x in args['data']])
            lock.release()
            return ret
