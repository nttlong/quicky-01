...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(ethnic_name, @name) or " + \
            "contains(ethnic_code, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_Ethnic.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Ethnic.update(
                data, 
                "ethnic_code == {0}", 
                args['data']['ethnic_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = Ethnic.display_list_ethnic().match("ethnic_code == {0}", args['data']['ethnic_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Ethnic.delete("ethnic_code in {0}",[x["ethnic_code"]for x in args['data']])
            lock.release()
            return ret
