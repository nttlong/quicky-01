...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_TrainCVRG.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_TrainCVRG.update(
                data, 
                "train_cvrg_code == {0}", 
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  = qmongo.models.HCSLS_TrainCVRG.delete("train_cvrg_code in {0}",[x["train_cvrg_code"]for x in args['data']])
            lock.release()
            return ret
