...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(nation_code, @name) or " + \
            "contains(nation_name, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =qmongo.models.HCSLS_Nation.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_Nation.update(
                data, 
                "nation_code == {0}", 
                args['data']['nation_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = Nation.display_list_nation().match("nation_code == {0}", args['data']['nation_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_Nation.delete("nation_code in {0}",[x["nation_code"]for x in args['data']])
            lock.release()
            return ret
