...
def get_email_template():
    collection = models.HCSSYS_EmailTemplate().aggregate()
...
def insert(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  models.HCSSYS_EmailTemplate().insert(args['data'])
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            del data['rec_id']
            ret  =  models.HCSSYS_EmailTemplate().update(
                data,
                "rec_id == {0}",
                args['data']['rec_id'])
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  models.HCSSYS_EmailTemplate().delete(
                "rec_id == {0}",
                args['data']['rec_id'])
            lock.release()
            return ret
