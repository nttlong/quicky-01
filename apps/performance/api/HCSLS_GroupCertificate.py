...
def get_list_with_searchtext(args):
    ...
    if(searchText != None):
        ret.match("contains(group_cer_name, @name) or " + \
            "contains(group_cer_code, @name) or " + \
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_GroupCertificate.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_GroupCertificate.update(
                data, 
                "group_cer_code == {0}", 
                args['data']['group_cer_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = GroupCertificate.display_list_group_cer().match("group_cer_code == {0}", args['data']['group_cer_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  = qmongo.models.HCSLS_GroupCertificate.delete("group_cer_code in {0}",[x["group_cer_code"]for x in args['data']])
            lock.release()
            return ret
