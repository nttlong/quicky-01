...
def get_list_with_searchtext(args):
    ...
    ret = display_list_employee_type()
...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_EmployeeType.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  =  qmongo.models.HCSLS_EmployeeType.update(
                data, 
                "emp_type_code == {0}", 
                args['data']['emp_type_code'])
            if ret['data'].raw_result['updatedExisting'] == True:
                ret.update(
                    item = display_list_employee_type().match("emp_type_code == {0}", args['data']['emp_type_code']).get_item()
                    )
            lock.release()
            return ret
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  =  qmongo.models.HCSLS_EmployeeType.delete("emp_type_code in {0}",[x["emp_type_code"]for x in args['data']])
            lock.release()
            return ret
