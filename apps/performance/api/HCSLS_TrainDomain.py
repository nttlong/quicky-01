...
def insert(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_TrainDomain.insert(data)
            lock.release()
            return ret
...
def update(args):
    try:
        ...
        if args['data'] != None:
            ...
            ret  = qmongo.models.HCSLS_TrainDomain.update(
                data, 
                "domain_code == {0}", 
...
def delete(args):
    try:
        ...
        ret = {}
        if args['data'] != None:
            ret  = qmongo.models.HCSLS_TrainDomain.delete("domain_code in {0}",[x["domain_code"]for x in args['data']])
            lock.release()
            return ret
